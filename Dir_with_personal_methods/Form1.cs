...
namespace Dir_with_personal_methods
{
    public partial class Form1 : Form
    {
        ...
        //Своя область 
        double a1 = 1.5, b1 = 1.75, c1 = 2.25, d1 = 2.75; //Мини квадратик 
            ...
            for (int j1 = 1; j1 < m; j1++)
            {
                for (int i1 = 1; i1 < n; i1++)
                {
                    ...
                    if ((i1 <= n / 4) && (j1 >= 3 * m / 4))
                        continue;
                    //Обходим вырезанную область 
                    if ((i1 >= n / 2) && (i1 <= 3 * n / 4) && (j1 >= m / 4) && (j1 <= 3 * m / 4))
                        continue;
                    
                    r[i1, j1] = (h2 * (v[i1 + 1, j1] + v[i1 - 1, j1]) + k2 * (v[i1, j1 + 1] + v[i1, j1 - 1])) + v[i1, j1] * a2 - f[i1, j1];
                    r_evk += r[i1, j1] * r[i1, j1];
                }
            }
            r_evk = Math.Sqrt(r_evk);
            label19.Text = Convert.ToString(r_evk);
        ...
        void z()
        {
            ...
            for (int j1 = 1; j1 < m + 2; j1++)
            {
                for (int i1 = 1; i1 < n + 2; i1++)
                {
                    if ((i1 < n / 4 + 1) && (j1 < m / 4 + 1))
                        dataGridView1.Rows[j1].Cells[i1].Value = 0;
                    //Обходим вырезанную область 
                    if ((i1 > n / 2 + 1) && (i1 < 3 * n / 4 + 1) && (j1 > m / 4 + 1) && (j1 < 3 * m / 4 + 1))
                        dataGridView1.Rows[j1].Cells[i1].Value = 0;
